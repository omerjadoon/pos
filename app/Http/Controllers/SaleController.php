...
class SaleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $input_date=$request->created_at;
        $data['created_at']=date("Y-m-d H:i:s",strtotime($input_date));
    }
    else{
        $data = $request->all();
    }
        ...
        if($data['pos']){
            ...
            $data['reference_no'] = 'sr-' . $data['created_at'] . '-'. date("his");
        ...
        foreach ($product_id as $i => $id) {
            $lims_p_data = Product::where('id', $id)->first();
            $total_cost += (($lims_p_data->cost)*($qty[$i])); 

        }

        $data['grand_cost'] = $total_cost;
        ...
        foreach ($product_id as $i => $id) {
            ...
            $product_sale['net_unit_cost'] = $lims_product_data->cost;
    ...
    public function getFeatured()
    {
        ...
        $lims_product_list = Product::where([
            ['is_active', true],
            ['featured', true]
        ])->select('products.id', 'products.name', 'products.code', 'products.image', 'products.is_variant' ,'products.produt_details')->get();
        ...
        foreach ($lims_product_list as $product) {
            if($product->is_variant) {
                ...
                foreach ($lims_product_variant_data as $key => $variant) {
                    ...
                    $images = explode(",", $product->image);
                    $data['image'][$index] = $images[0];
                    $data['details'][$index] = $product->produt_details;
                    $index++;
                }
            }
            else {
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product[] = $lims_product_data->product_details;
        return $product;
    ...
    {
         $data = $request->except('document','created_at');
        $input_date=$request->created_at;
        $data['created_at']=date("Y-m-d H:i:s",strtotime($input_date));
       // $data = $request->except('document');
        //return dd($data);
